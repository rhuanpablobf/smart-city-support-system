...
const ActiveChatState: React.FC<ActiveChatStateProps> = ({ conversationId }) => {
  ...
  useEffect(() => {
    ...
    const subscriptionIds = realtimeService.subscribeToTables(['messages'], 'INSERT', (payload) => {
    ...
    const conversationSubscriptionIds = realtimeService.subscribeToTables(['conversations'], 'UPDATE', (payload) => {
      if (payload.new && 
          'id' in payload.new && 
